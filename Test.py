...
def run_quiz(questions, Interpretation):
     ...
     dfpython = pd.DataFrame([petitdico])
     #print(dfpython)

     groupe1 = []
     for x in dfpython['Q1']:
        if x not in ["a", "b", "c"]:
            x = np.nan
            groupe1.append(x)
        else :
     ...
     groupe10 = []
     for x in dfpython['Q10']:
        if x not in ["1", "2", "3"]:
            x = np.nan
            groupe10.append(x)
        else :
            groupe10.append(x)
     dfpython['Q10'] = groupe10


     dfpython = dfpython.apply(lambda x:x.fillna(x.value_counts()))


     for col in ["Q1", "Q2", "Q3", "Q4", "Q5", "Q6", "Q7", "Q8", "Q9", "Q10"]:
        dfpython[col] = LabelEncoder().fit_transform(dfpython[col])


     knn = joblib.load(r"modele_knn.pkl")
     prediction = knn.predict(dfpython).reshape(-1,1)

     print('\n--------- Prédiction grâce au modèle KNN ---------\n')
     print(prediction)
     print('\n--------- Fin de la prédiction grâce au modèle KNN ---------\n')
